...
class App(wx.App):
    def OnInit(self):
        self.frame = wx.Frame(None, -1, title=u'Conjunto de Mandelbrot', size=(850,550))
        self.crear_gui_masfigura(self.frame)
        self.frame.Show()        
        self.ax = self.fig.add_subplot(111, navigate=True)
        self.ejecutar(self)
        return True
    ...
    def crear_gui_masfigura(self, parent):
        ...
        self.caja_Xmin = wx.TextCtrl(panelbut, -1, value='-2.5', size=(-1,-1))
        ...
        self.caja_Xmax = wx.TextCtrl(panelbut, -1, value='1.5', size=(-1,-1))
        ...
        self.caja_fun = wx.TextCtrl(panelbut, -1, value='mandelbrot',size=(-1,-1))
        ...
        label8 = wx.StaticText(panelbut, -1, '  Paleta de colores:')
    ...
    def ejecutar(self,event):
        ...
        if (er == 0):
            ...
            z = zeros_like(c) # Creamos una matriz de ceros del mismo tamano que c
            for n in xrange(self.maxiter):
                indices = (abs(z) <= 10)  # Conjunto de indices tal que |z| <= 10 (10 para mejor dibujado, 2 es sufic)
                z[indices] = feval(self.fun, z[indices], c[indices]) # Aplicamos la funcion que hemos definido
                iters[indices] = n
            self.ax.cla()
    ...
    def restaurar(self,event):
        ...
        self.caja_Xmin.SetValue('-2.5')
        self.caja_Xmax.SetValue('1.5')
    ...
    def about(self, event):
        descripcion = u"""Este programa permite la visualización del conjunto de Mandelbrot, pudiendo definirse nuevas
funciones para visualizar dicho conjunto y usarse varias paletas de colores y distintos ajustes de visualización."""
        inform = wx.AboutDialogInfo()
        inform.SetIcon(wx.Icon('iconomand.png', wx.BITMAP_TYPE_PNG))
